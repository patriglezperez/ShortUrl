...
async function findFullUrl(req, res) {
  const { shortUrl } = await req.params; // console.log("esto es lo que buscamos", shortUrl); // const result = await Url.find({}).where({ shortUrl: shortUrl });
...
module.exports = findFullUrl; // async function findFullUrl(client, nameOfAddress) {
